...
def normalize_train_test_movies(
    ...
    Note: This function uses numpy for efficient computation of statistics on large arrays.
    """
    # Convert to float32 for consistent precision
    train_array = train.astype(np.float32)
    test_array = test.astype(np.float32)
    ...
    train_video_preproc = (train_array - train_mean) / train_std
    test_video_preproc = (test_array - train_mean) / train_std
    
    return (
        train_video_preproc,
        test_video_preproc,
        {"norm_mean": float(train_mean), "norm_std": float(train_std)},
    )
