...
from .base import MoviesTrainTestSplit, ResponsesTrainTestSplit
...
class TrainTestStimuliProcessor:
    ...
    def _normalize_stimulus(self, stimulus: np.ndarray) -> np.ndarray:
        # Using numpy for normalization
        stimulus_array = stimulus.astype(np.float32)
        normalized_array = (stimulus_array - self._norm_mean) / self._norm_std
