...
# Locate the repository root. Note sbatch COPIES this script to a node-local spool directory
# (/var/spool/slurmd/job<id>/), so ${BASH_SOURCE[0]} is useless under SLURM -- prefer an explicit
# REPO, then the submission directory, and only then the script's own location (direct execution).
for candidate in "${REPO:-}" "${SLURM_SUBMIT_DIR:-}" \
                 "$(cd "$(dirname "${BASH_SOURCE[0]}")" 2>/dev/null && pwd)"; do
  if [[ -n "$candidate" && -d "$candidate/configs" && -f "$candidate/pyproject.toml" ]]; then
    REPO="$candidate"
    break
  fi
done
if [[ -z "${REPO:-}" || ! -d "$REPO/configs" ]]; then
  echo "ERROR: could not locate the open-retina repo root. Submit from it, or set REPO=/path/to/open-retina." >&2
  exit 1
fi
echo "=== repo root: $REPO"
cd "$REPO" || exit 1
