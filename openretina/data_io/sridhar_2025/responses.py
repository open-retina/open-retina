...
def load_responses(base_path, files, stimulus_seed = 0, excluded_cells:dict = None,
                   cell_index: int = None):
    responses = {}
    for session_id, file in files.items():
        ...
        if cell_index is not None:
            ...
            train_responses, excluded_cells[session_id], axis=0
            )
            test_responses = np.delete(
                test_responses, excluded_cells[session_id], axis=0
            )
        ...
        responses[session_id] = {'train_responses': train_responses,
