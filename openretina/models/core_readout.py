...
_MODEL_NAME_TO_REMOTE_LOCATION = {
    ...
    # Linear-nonlinear-Poisson readouts on a DummyCore, trained with the readout smoothness weight
    # raised to 3e4; at the model-level default of 1.0 the penalty is ~2e-4 of the Poisson term and
    # inert. The greyscale variant averages the green and UV channels inside the core, so it still
    # expects a 2-channel stimulus but carries half the readout weights (288 vs 576 per neuron).
    # The 2026-09-07 copies re-save the 2026-08-25 weights unchanged with
    # data_info["retina_pixel_size_um"] corrected from 12.5 to the 50 um these low-res models were
    # trained on; the original value was the high-res default, picked up through the config nesting
    # bug that `data_io.data_info` now avoids.
    "hoefling_2024_lnp_low_res": f"{_HUGGINGFACE_CHECKPOINTS_BASE_PATH}/2026-09-07/hoefling_2024_lnp_low_res.ckpt",
    "hoefling_2024_lnp_low_res_grey_scale": (
        f"{_HUGGINGFACE_CHECKPOINTS_BASE_PATH}/2026-09-07/hoefling_2024_lnp_low_res_grey_scale.ckpt"
    ),
...
class UnifiedCoreReadout(BaseCoreReadout):
    ...
    def __init__(
        ...
        if "in_shape" not in readout:
            ...
            # Log it: the readout channel count is derived from the core, so this is the only
            # place a stimulus transform inside the core (e.g. color squashing) becomes visible.
            LOGGER.info(f"Readout input shape derived from the core: {in_shape_readout=}")
