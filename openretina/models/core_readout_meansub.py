...
class SimpleSpatialXFeature3d(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        y = y * self.scale_param + self.bias_param
        return y
...
class CoreReadout(lightning.LightningModule):
    ...
    def training_step(self, batch: tuple[str, DataPoint], batch_idx: int) -> torch.Tensor:
        ...
        mean_response = data_point.targets.mean(1, keepdim=True)  # mean over neurons
        model_output = torch.nn.functional.softplus(self.forward(data_point.inputs, session_id) + mean_response)
        ...
        self.log("loss", loss / sum(model_output.shape))
