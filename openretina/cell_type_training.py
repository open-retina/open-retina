...
from .hoefling_2024.constants import CLIP_LENGTH, NUM_CLIPS, NUM_VAL_CLIPS, SCENE_LENGTH
from .hoefling_2024.data_io import (
    MoviesDict,
    gen_start_indices,
    get_all_movie_combinations,
)
from .hoefling_2024.models import ParametricFactorizedBatchConv3dCore
...
class SingleCellMovieSampler(Sampler):
    def __init__(
        self,
        clip_start_indices: Float[np.ndarray, " n_indices"],
        n_neurons,
        split,
        chunk_size,
        scene_length=None,
    ...
    def __iter__(self):
        if self.split == "train" and (self.scene_length != self.chunk_size):
            ...
            shifted_indices = gen_shifts(
                np.arange(0, max(self.start_indices), self.scene_length),
                self.start_indices,
                self.chunk_size,
            )
            shuffle = True
        ...
        # Creates sampling indices for each neuron and clip start combination!
...
class CellTypeNeuronData(NeuronData):
    ...
    def compute_validation_responses(self):
        # Initialise train and validation responses for all neurons
        ...
        for neuron_index, neuron_scan_seq in enumerate(self.scan_sequence_idx):
            ...
            for i, ind1 in enumerate(self.val_clip_idx):
                ...
                self.responses_val[
                    i * self.clip_length : (i + 1) * self.clip_length, neuron_index
...
def get_movie_cell_types_dataloader(
    movies: dict,
    responses: Float[np.ndarray, "n_neurons n_frames"],
    roi_ids: Float[np.ndarray, " n_neurons"],
    roi_coords: Float[np.ndarray, "n_neurons 2"],
    group_assignment: Float[np.ndarray, " n_neurons"],
    split: str,
    start_indices: Float[np.ndarray, " n_indices"],
    eye: Float[np.ndarray, " n_neurons"],
    scan_sequence: Float[np.ndarray, " n_neurons"],
    chunk_size: int = 50,
    batch_size: int = 32,
    ...
    **kwargs,
    ...
    sampler = SingleCellMovieSampler(
        start_indices,
        n_neurons=len(group_assignment),
        split=split,
...
def calculate_chirp_features(chirp_trace: Float[np.ndarray, "n_neurons n_timepoints"]):
    ...
    return np.concatenate([spectral_centroid_value, spectral_flatness_value, mean_value, std_value, range_value])


def generate_cell_barcodes(responses_dict):
    cell_barcodes = {}

    for field_id in responses_dict:
        field_barcode = {
            "cell_types": responses_dict[field_id]["group_assignment"],
            "chirp_features": calculate_chirp_features(responses_dict[field_id]["chirp_preprocessed_traces"]),
            "roi_size": responses_dict[field_id]["roi_size"],
            ...
            "temporal_nasal_pos_um": responses_dict[field_id]["temporal_nasal_pos_um"],
            "ventral_dorsal_pos_um": responses_dict[field_id]["ventral_dorsal_pos_um"],
        }
        cell_barcodes[field_id] = field_barcode

        # Need to apply a normalisation to all of these at some point.
...
class ReadoutWeightShifter(nn.Module):
    def __init__(self, num_numerical_features, categorical_vocab_sizes, categorical_embedding_dims, output_dim):
        ...
        self.embeddings = nn.ModuleList(
            [nn.Embedding(vocab_size, categorical_embedding_dims) for vocab_size in categorical_vocab_sizes]
        )

        # Dense layers for numerical features
        self.num_numerical_features = num_numerical_features
        self.fc1 = nn.Linear(num_numerical_features + categorical_embedding_dims * len(categorical_vocab_sizes), 64)
    ...
    def forward(self, numerical_input, categorical_inputs):
        ...
        # Forward pass through dense layers with BatchNorm
        x = F.relu(self.bn1(self.fc1(x)))
        x = F.relu(self.fc2(x))
        return self.fc3(x)


class FrozenFactorisedReadout2d(nn.Module):
    def __init__(
        self,
        in_shape,
        outdims,
...
class ShifterVideoEncoder(nn.Module):
    ...
    def forward(self, x, cell_barcodes: List[torch.Tensor], data_key=None, detach_core=False, **kwargs):
        ...
        feature_weights = self.readout_shifter(*cell_barcodes)
