...
def poisson_stop3d(model, loader, avg=True, device='cpu'):
    ...
    for data_key in loader:
        ...
        ret = output - target * np.log(output + 1e-12)
        if np.any(np.isnan(ret)):
            warnings.warn(' {}% NaNs in poisson_stop3d'.format(np.isnan(ret).mean() * 100))
...
def evaluate_fev(model, loader, device="cpu", ddof=0):
    ...
    for i, data_key in enumerate(loader.keys()):
        ...
        test_responses_by_trial = \
            loader[data_key].dataset.test_responses_by_trial.numpy()  # shape is neurons x repetitions x time
        cropped_responses = crop_responses(test_responses_by_trial,
                                           predictions)

        noise_variance = \
            np.mean(    # mean across time
                np.var(  # variance across repetitions
                    cropped_responses, axis=1, ddof=ddof
                ), axis=-1
            )
        total_variance = \
            np.var(
                cropped_responses, axis=(-1, -2)
            )


        flattened_responses = \
            cropped_responses.reshape(
