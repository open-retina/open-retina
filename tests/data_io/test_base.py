...
class TestMoviesTrainTestSplit:
    ...
    def test_validation_errors(self, valid_train_movie, valid_test_movie):
        ...
        with pytest.raises(ValueError, match="Exactly one of test_dict and test should be set"):
            MoviesTrainTestSplit(train=valid_train_movie, test=valid_test_movie, test_dict={"other": valid_test_movie})
...
class TestResponsesTrainTestSplit:
    ...
    def test_validation_errors(self, valid_train_responses, valid_test_responses):
        ...
        with pytest.raises(ValueError, match="Exactly one of test_dict and test should be set"):
            ResponsesTrainTestSplit(
                train=valid_train_responses, test=valid_test_responses, test_dict={"other": valid_test_responses}
            )
    ...
    def test_check_matching_stimulus(self, valid_train_responses, valid_test_responses):
        ...
        responses_split = ResponsesTrainTestSplit(
            train=valid_train_responses, test=valid_test_responses, stim_id="stim1"
        )
        movies_split = MoviesTrainTestSplit(train=train_movie, test=test_movie, stim_id="stim2")
...
def test_get_n_neurons_per_session():
    ...
    responses_dict = {
        "session1": ResponsesTrainTestSplit(train=np.random.randn(10, 100), test=np.random.randn(10, 50)),
        "session2": ResponsesTrainTestSplit(train=np.random.randn(15, 100), test=np.random.randn(15, 50)),
    }
...
def test_compute_data_info():
    ...
    movies_split = MoviesTrainTestSplit(train=train_movie, test=test_movie, norm_mean=0.5, norm_std=1.2)

    responses_dict = {
        "session1": ResponsesTrainTestSplit(
            train=np.random.randn(10, 100), test=np.random.randn(10, 50), session_kwargs={"param1": "value1"}
        ),
        "session2": ResponsesTrainTestSplit(
            train=np.random.randn(15, 100), test=np.random.randn(15, 50), session_kwargs={"param2": "value2"}
        ),
    }
    ...
    assert result["sessions_kwargs"] == {"session1": {"param1": "value1"}, "session2": {"param2": "value2"}}
    assert result["movie_norm_dict"] == {"default": {"norm_mean": 0.5, "norm_std": 1.2}}

    # Test with dictionary of MoviesTrainTestSplit
    movies_dict = {
        "movie1": MoviesTrainTestSplit(train=train_movie, test=test_movie, norm_mean=0.5, norm_std=1.2),
        "movie2": MoviesTrainTestSplit(train=train_movie, test=test_movie, norm_mean=0.3, norm_std=0.9),
    }
